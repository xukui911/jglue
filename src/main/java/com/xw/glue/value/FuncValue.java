...
public class FuncValue extends Value<Object> {
	List<Word> words;
	IFunc func;
	...
	FuncExec exec;
	public FuncValue(List<Word> words, JProperties properties, Grammar parentGrammar) {
		super(words);
		this.words = words;
	...
	public class ParamExec implements FuncExec {
		...
		public Result value(GrammerContext context) {
			...
			if (oFunc!=null && oFunc instanceof RuntimeFunc) {
				...
				if (funcNameParamValue!=null) {
					FuncValue.this.exec = new SubFuncExec(funcNameParamValue);
				} else {
					if(func==null) {
						FuncValue.this.func = FuncFactory.getInstance().getFunc(namespace, funcName);
					}
					if(func==null) {
						FuncValue.this.func = FuncFactory.getInstance().getInsFunc(funcName);
					}
					FuncValue.this.exec = new GlobleFuncExec();					
				}
				return FuncValue.this.exec.value(context);
			}
		}
	}
